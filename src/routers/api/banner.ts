...
class Banner {
    ...
    private GetBanners = async(req: express.Request, res: express.Response) => {
        let banner: any = await this.bannerController.getAllBanners(req.query.outletChainID, req.query?.isAdmin === "1");
        await res.send(banner);
    }

    private GetBanner = async(req: express.Request, res: express.Response) => {
        let banner: any = await this.bannerController.getSingleBanner(req.query.bannerId);
        await res.send(banner);
    }
    ...
    private UpdateBrand = async(req: express.Request, res: express.Response) => {
        let banner: any = await this.bannerController.updateBanner(req.query.bannerId, req.body);
        await res.send(banner);
    }

    private DeleteBrand = async(req: express.Request, res: express.Response) => {
        let banner: any = await this.bannerController.deleteBanner(req.query.bannerId);
        await res.send(banner)
    }
}
