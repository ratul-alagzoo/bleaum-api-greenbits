...
class ProductController {
  ...
  public getAllProducts = async () => {
    let returnData = {};
    await Product.find({ missing: false })
      .then(async (res: any) => {
        // console.log(res);
        if (!res) {
          returnData = {
  ...
  public getMissingProduct = async () => {
    let returnData = {};
    await Product.find({ missing: true })
      .then(async (res: any) => {
        // console.log(res);
        if (!res || res.length === 0) {
          returnData = {
  ...
  public addMultipleInventoryV1 = async (products: any[]) => {
    try{
      ...
      //get all products
      const productsKeyMap: Record<string, any> = products.reduce((acc, el) => ({...acc, [el.productID as string ]: true }), {});
      ...
      let featuredList: Record<string, boolean> = existingProducts.reduce((acc, el) => ({...acc, [el.productID as string ]: !!el.featuredProduct}), {});
      const productsToDelete: Record<string, boolean> = Object.keys(featuredList).reduce((acc, el) => {
  ...
  public addMultipleInventory = async (Body: any) => {
    ...
    await Inventory.insertMany(sampleArray)
      .then(async (response) => {
    ...
    Inventory.remove({ variants: { $exists: false } });
